 ...
 */
@Component
public class CaUtils implements InitializingBean {
	...
	private static boolean isTryUrlGet = false;

	@Override
	public void afterPropertiesSet() throws Exception {
		isTryUrlGet = !springUtils.isProd();
	}
	...
	public static String getAccessToken(CaConfig caConfig, boolean isForce) throws CaTokenException{
		return getAccessToken(caConfig, isForce, isTryUrlGet);
	}
	/**
	 * 获取AccessToken
	 * @param isForce 是否强制获取,不走缓存
	 * @param isTryUrlGet 是否尝试通过url获取
	 * @return AccessToken
	 */
	public static String getAccessToken(CaConfig caConfig, boolean isForce, boolean isTryUrlGet) throws CaTokenException {
		// 非正式环境的通过url共享token,保证性能和安全性,下面都使用局部变量
		if(isTryUrlGet){
			try {
				RestTemplate restTemplate = new RestTemplate();
				String hankCaGetAccessToken = AesCryptTools.encrypt("caGetAccessTokenDate:" + System.currentTimeMillis(), "hankCaGetAccessToken");
				hankCaGetAccessToken = URLEncoder.encode(hankCaGetAccessToken, CharEncoding.UTF_8);
				ResponseEntity<String> objectResponseEntity = restTemplate
						.postForEntity("https://tcm-saas-test.nhf.cn/api/wjj-saas-system/nonProd/ca/getAccessToken/wjjGmOpenToken/"+hankCaGetAccessToken, isForce, String.class);
				String accessToken = objectResponseEntity.getBody();
				if(StringUtils.isNotBlank(accessToken)){
					return accessToken;
				}
			}catch (Exception e){
				logger.error("getAccessToken tryUrlGet error.", e);
			}
		}
		StringRedisTemplate stringRedisTemplate = null;
		try {
			stringRedisTemplate = CaSpringUtils.getBean(StringRedisTemplate.class);
		} catch (Exception e) {
