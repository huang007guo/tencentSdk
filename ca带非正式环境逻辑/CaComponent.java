...
//                        recipeInfoIn.setUnitOf(); // @菲凡 非必传就不用管了 20201224
    ...
    private void pharmacistAuditPrescriptionPass(MedicalHistoryPrescription medicalHistoryPrescription){
        // 如果已经同步过的不需要在同步
        if(StringUtils.isNotBlank(medicalHistoryPrescription.getCaPharmacistUniqueId())){
            return;
        }
...
//                        recipeInfoIn.setUnitOf(); // @菲凡 非必传就不用管了 20201224
        ...
        if (CaConst.SIGN_CALL_BACK_STATUS_SUCCESS.equals(signCallbackIn.getBody().getStatus())) {
            ...
            if(sysUser == null){
                if(springUtils.isProd()) {
                    ...
                    // 测试环境有可能是转发的
                    log.warn("ca caSignNotify fail 系统中不存在当前openid,对应的医生, signCallbackIn:{}", JSON.toJSONString(signCallbackIn));
                }
            ...
            if (CollectionUtils.isNotEmpty(docSignPrescriptionList)) {
                // 医生签名成功
                doctorSignSuccess(docSignPrescriptionList, uniqueId, openId, sysUser);
            }else{
                // 药师签名的处方列表
                List<MedicalHistoryPrescription> pharmacistSignPrescriptionList = medicalHistoryPrescriptionService.lambdaQuery()
                        ...
                        .eq(MedicalHistoryPrescription::getStatus, MedicalHistoryConst.PRESCRIPTION_STATUS_WAIT_AUDIT)
                        .list();
                if(CollectionUtils.isEmpty(pharmacistSignPrescriptionList)){
                    if(springUtils.isProd()) {
                        log.error("ca caSignNotify fail 没有匹配到对应的处方(医生,药师), signCallbackIn:{}", JSON.toJSONString(signCallbackIn));
                    }else{
                        // 测试环境有可能是转发的
                        log.warn("ca caSignNotify fail 没有匹配到对应的处方(医生,药师), signCallbackIn:{}", JSON.toJSONString(signCallbackIn));
                    }
                    return CallbackOut.fail("没有匹配到对应的处方");
                }
                pharmacistSignSuccess(pharmacistSignPrescriptionList, uniqueId, openId, sysUser);
            }
        }
    ...
    /**
     * 医生签名成功
     * @param docSignPrescriptionList
     * @param uniqueId
     * @param openId
     * @param sysUser
     * @throws CommonException
     */
    private void doctorSignSuccess(List<MedicalHistoryPrescription> docSignPrescriptionList, String uniqueId, String openId, SysUser sysUser) throws CommonException{
            ...
            if(assignPharmacistSysUser != null){
                ...
                TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
                    ...
                    public void afterCommit() {
                        MedicalHistoryPrescription medicalHistoryPrescriptionNew = medicalHistoryPrescriptionService.getById(medicalHistoryPrescription.getId());
                        if(
                                MedicalHistoryConst.PRESCRIPTION_STATUS_WAIT_AUDIT.equals(medicalHistoryPrescriptionNew.getStatus()) &&
                                        medicalHistoryPrescriptionNew.getCaPharmacistUserId() != null &&
                                        StringUtils.isNotBlank(medicalHistoryPrescriptionNew.getCaPharmacistSignOpenid())
                        ) {
                            // 是否发通知(如果有自动审核，就不用发短信了)
                            ...
                            if (saasCommonConfigurationVO != null && (new Integer(10)).equals(saasCommonConfigurationVO.getPharmacistAutoAuditConfig())) {
                                // 请求Ca二次判断是否开启了
                                SelfSignGetGrantResultOut selfSignGetGrantResultOut = caFeignClient.selfSignGetGrantResult(new BaseIn<>(new SelfSignGetGrantResultIn(medicalHistoryPrescriptionNew.getCaPharmacistSignOpenid(), null)));
                                if (selfSignGetGrantResultOut.getData()) {
                                    // 自动审核通过,提交数据到ca自动签名
                                    pharmacistAuditPrescriptionPass(medicalHistoryPrescriptionNew);
                                    // 如果有自动审核，就不用发短信了
                                    isNotify = false;
                                }
                            }
                            if(isNotify) {
                                // 通知药师审方的短信、APP推送
                                reminderService.assignPharmacistNotify(medicalHistoryPrescriptionNew.getCaPharmacistUserId(), medicalHistory.getEmployeeName());
                            }
                        }
                    }
                });
    ...
    /**
     * 药师签名成功
     * @param pharmacistSignPrescriptionList
     * @param uniqueId
     * @param openId
     * @param sysUser
     * @throws CommonException
     */
    private void pharmacistSignSuccess(List<MedicalHistoryPrescription> pharmacistSignPrescriptionList, String uniqueId, String openId, SysUser sysUser) throws CommonException{
